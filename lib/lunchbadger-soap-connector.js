...
/**
* This function will be moved out to the test cases
* in the next iteration (with suitable refactoring)
*/
function testinvoke(dataSource) {

  if (!dataSource) {
    debug('testinvoke: undefined dataSource object');
  ...
  process.nextTick( function() {
    connector.connect(function(err, callback) {
      ...
      soapreq.invoke("OrteLookup", "OrteLookupSoap", "GetPrefixedEntries",
                      function(err, result) {
                        ...
                        console.log("SOAP Response = " + JSON.stringify(result));
                      });
